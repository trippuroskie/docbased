...
export function buildContextBlock(hits: SearchHit[]): {
  context: string;
  sources: Array<{ n: number; documentId: string; chunkId: string; title: string; headingPath: string[] }>;
} {
...
export function buildDocumentIndex(entries: IndexEntry[]): string {
  ...
  // Group by space, then list "title  —  path" lines so the model can reason
  // about hierarchy without us spamming heading info for every chunk.
...
// Citation tags look like <cite source="3"/> or <cite source='3'/>.
// We parse them out of the streamed assistant text into structured citations.
